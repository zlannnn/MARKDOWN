...
细节和理论可以看文章,文中提到的2prime大佬在知乎的总结或者本文末链接的两篇文章。这里就说得稍微笼统易懂一些。
...
我们对比的时候基本上把trick去了个精光，因为确实看了很多文章觉得work，后面发现不知道是什么trick在work。至于加了trick会怎样，我们心里其实也没底。

#### 到底是什么在work
我个人猜测最重要的地方还是高阶办法总是在最初的input位置shortcut出去x。从误差传播的角度，反复shortcut中间层的数值确实不大合理。

还有一个很明显的事情是高阶办法中千奇百怪的，对中间层的scale系数。目前网络中在shortcut中间层的时候，还是以直接+x为主。不过我个人测了一下，这个问题似乎不是特别的关键。

#### 堆叠的不只是block
我最想说的事情是这个。堆叠block只是说动机的时候，觉得现在层数动不动两三百，但总体上还是缺乏对网络的宏观链接。但这种堆叠方式绝对不仅仅只是backbone。

很多head和neck也可以参考这种思想。我觉得本质上还是中间层的状态shortcut出去就是需要一些误差控制。所以层数多起来这种思想总是有点作用的。但我们确实没那个精力去跑太多task去验证，所以文中也没敢乱说。只等以后慢慢做吧。

#### 可视化权重surface或者loss surface
这件事蛮有意义的，ResNet的Loss surface比plain要顺滑太多了。虽然没有做可视化，但我有较大的自信，高阶办法链接的ResNet的loss surface会更加平滑。
